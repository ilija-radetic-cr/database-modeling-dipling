...
func validateV06(bundle *dsl.Bundle) Result {
	validator := validatorV06{
		bundle:         bundle,
		doc:            bundle.Document,
...
// ValidateV06Bundle validates an in-memory logical candidate before it is
// written to a project revision.
func ValidateV06Bundle(bundle *dsl.Bundle) Result {
	if bundle == nil || bundle.Document == nil || bundle.SourceUnits == nil || bundle.ReviewDecisions == nil {
		return Result{Errors: []string{"in-memory DB-DSL v0.6 bundle is incomplete"}}
	}
	...
	return validateV06(&candidate)
}

type validatorV06 struct {
	bundle *dsl.Bundle
	doc    *dsl.Document
	errors []string
...
func (v *validatorV06) run() {
	...
	v.validateIndexes()
	v.validateEvidence()
	v.validateReviewDecisions()
	v.validateReferenceableTargets()
}

func (v *validatorV06) buildBundleIndexes() {
...
func (v *validatorV06) validateTopLevel() {
	for _, key := range []string{"dsl", "model", "source", "entities", "relationships", "constraints", "import_specs", "state_machines", "derived_views", "file_specs"} {
	...
	if v.doc.DSL.Version != "0.6" {
		v.add("dsl.version must be 0.6 for v0.6 validator")
	}
	if v.doc.Model.ID == "" {
...
func (v *validatorV06) buildModelIndexes() {
	...
	indexIDs := make([]string, 0, len(v.doc.Indexes))
	for _, index := range v.doc.Indexes {
		indexIDs = append(indexIDs, index.ID)
	}
	v.indexIDs("index", "indexes", indexIDs)
}

func (v *validatorV06) indexIDs(label, kind string, ids []string) {
...
func (v *validatorV06) validateEntities() {
	...
	for _, entity := range v.doc.Entities {
		...
		for _, attribute := range entity.Attributes {
			...
			if (entity.Kind == "regular" || entity.Kind == "lookup") && strings.EqualFold(attribute.ID, "id") {
				v.add("%s uses reserved generated primary key id; omit this attribute because DB-DSL v0.6 generates it for %s entities", attrPrefix, entity.Kind)
			}
...
func (v *validatorV06) validateRelationships() {
...
func (v *validatorV06) buildGeneratedFKs() {
...
func (v *validatorV06) validateConstraints() {
...
func (v *validatorV06) validateConditionalRequired(prefix string, constraint dsl.Constraint) {
...
func (v *validatorV06) validateImportSpecs() {
...
func (v *validatorV06) validateStateMachines() {
...
func (v *validatorV06) validateDerivedViews() {
...
func (v *validatorV06) validateIndexes() {
	for _, index := range v.doc.Indexes {
		prefix := fmt.Sprintf("index %s", displayID(index.ID))
		v.requireString(prefix, "owner", index.Owner)
		v.requireString(prefix, "description", index.Description)
		if index.Owner != "" && !v.hasEntity(index.Owner) {
			v.add("%s references unknown owner entity %s", prefix, index.Owner)
			continue
		}
...
func (v *validatorV06) validateFileSpecs() {
...
func (v *validatorV06) validateEvidence() {
	for _, entity := range v.doc.Entities {
	...
	for _, index := range v.doc.Indexes {
		v.validateEvidenceObject(fmt.Sprintf("index %s", displayID(index.ID)), index.Evidence)
	}
}

func (v *validatorV06) validateEvidenceObject(prefix string, evidence dsl.Evidence) {
...
func (v *validatorV06) validateReviewDecisions() {
	for _, review := range v.bundle.ReviewDecisions.ReviewDecisions {
...
func (v *validatorV06) validateReferenceableTargets() {
	for _, relationship := range v.doc.Relationships {
...
func (v *validatorV06) validateConstraintField(prefix, owner, field string) dsl.ConstraintReference {
...
func (v *validatorV06) validateSingleConstraintField(prefix, owner, field string) dsl.ConstraintReference {
...
func (v *validatorV06) validateImportTarget(prefix, target string) {
...
func (v *validatorV06) requireSourceUnit(prefix, sourceUnit string) {
	if sourceUnit == "" {
...
func (v *validatorV06) hasEntity(id string) bool {
	_, ok := v.entityByID[id]
	return ok
}

func (v *validatorV06) attribute(entityID, attributeID string) *dsl.Attribute {
...
func (v *validatorV06) requireString(prefix, field, value string) {
	if value == "" {
		v.add("%s.%s is required", prefix, field)
	}
}

func (v *validatorV06) add(format string, args ...any) {
